...
void test_whisper_audio_integration() {
  ...
  std::cout << "Loading real audio file from assets..." << std::endl;

  std::string audio_file_path = "../../../assets/002-01.wav";  // Path to larger audio file
  std::vector<float> test_audio;

  try {
    test_audio = AudioDecoder::decode_audio(audio_file_path, WHISPER_SAMPLE_RATE);
    if (test_audio.empty()) {
      std::cout << "⚠ Failed to load 002-01.wav, trying 001.wav..." << std::endl;

      // Try the smaller file as backup
      audio_file_path = "../../../assets/001.wav";
      test_audio = AudioDecoder::decode_audio(audio_file_path, WHISPER_SAMPLE_RATE);

      if (test_audio.empty()) {
        std::cout << "⚠ Failed to load both audio files, falling back to synthetic audio" << std::endl;
        ...
        std::cout << "✓ Successfully loaded backup audio file (" << test_audio.size() << " samples, "
                  << (test_audio.size() / float(WHISPER_SAMPLE_RATE)) << " seconds)" << std::endl;
      }
    } else {
      std::cout << "✓ Successfully loaded large audio file (" << test_audio.size() << " samples, "
                << (test_audio.size() / float(WHISPER_SAMPLE_RATE)) << " seconds)" << std::endl;
  ...
  // Test the AudioDecoder with our loaded file
  std::cout << "✓ AudioDecoder successfully loaded: " << audio_file_path << std::endl;
  std::cout << "Audio properties:" << std::endl;
  std::cout << "  - Samples: " << test_audio.size() << std::endl;
  std::cout << "  - Duration: " << (test_audio.size() / float(WHISPER_SAMPLE_RATE)) << " seconds" << std::endl;
  std::cout << "  - Sample Rate: " << WHISPER_SAMPLE_RATE << " Hz" << std::endl;

  // Show some sample values
  if (test_audio.size() >= 10) {
    std::cout << "  - First 10 samples: ";
    for (int i = 0; i < 10; ++i) {
      std::cout << std::fixed << std::setprecision(3) << test_audio[i] << " ";
    }
    std::cout << std::endl;
  }
...
/**
 * Usage example for whisper audio processing with real files
 */
void demonstrate_usage() {
  std::cout << "\n=== Usage Example ===" << std::endl;

  std::cout << "// Example usage in your application with real audio files:" << std::endl;
  std::cout << "// 1. Load audio file from assets:" << std::endl;
  std::cout << "//    auto audio = AudioDecoder::decode_audio(\"assets/002-01.wav\", 16000);" << std::endl;
  std::cout << "//    // For large files, consider processing in chunks" << std::endl;
  std::cout << "//" << std::endl;
  std::cout << "// 2. Preprocess audio with whisper-compatible functions:" << std::endl;
  ...
  std::cout << "//" << std::endl;
  std::cout << "// 3. Extract features for whisper model:" << std::endl;
  std::cout << "//    FeatureExtractor extractor;" << std::endl;
  std::cout << "//    auto features = extractor.extract(filtered);" << std::endl;
  std::cout << "//" << std::endl;
  std::cout << "// 4. Pass features to your whisper model:" << std::endl;
  ...
  std::cout << "// - Large WAV files (like assets/002-01.wav - 28MB)" << std::endl;
  std::cout << "// - Smaller WAV files (like assets/001.wav, test.wav)" << std::endl;
  std::cout << "// - Automatic resampling to 16kHz if needed" << std::endl;
  std::cout << "// - Mono audio processing optimized for speech" << std::endl;
