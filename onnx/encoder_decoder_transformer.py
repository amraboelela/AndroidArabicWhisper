...
class EncoderDecoderTransformer(nn.Module):
    ...
    def decode(self, text_ids, encoder_output, tgt_key_padding_mask: torch.Tensor = None, memory_key_padding_mask: torch.Tensor = None):
        ...
        positions = torch.arange(tgt_len, device=text_ids.device).clamp(max=self.decoder_pos_embedding.num_embeddings - 1)
    ...
    def forward(self, mel_features=None, text_ids=None, encoder_output=None,
        ...
        return self.decode(text_ids, encoder_output, tgt_key_padding_mask=tgt_key_padding_mask,
    ...
    def generate(self, mel_features, max_new_tokens=50, temperature=1.0, min_tokens=1,
        ...
        with torch.no_grad():
            ...
            for step in range(max_new_tokens):
                logits = self.decode(text_ids, encoder_output)
...
class WhisperEncoderBlock(nn.Module):
    ...
    def forward(self, x):
        x = x + self.attn(self.attn_ln(x), self.attn_ln(x))
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_output, tgt_key_padding_mask=None, memory_key_padding_mask=None):
        # Pre-norm residual (same as encoder and Whisper)
        x = x + self.self_attn(self.ln1(x), self.ln1(x), key_padding_mask=tgt_key_padding_mask)
        x = x + self.cross_attn(self.ln2(x), encoder_output, key_padding_mask=memory_key_padding_mask)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key_value, key_padding_mask=None):
        ...
        if self.causal:
            causal_mask = torch.triu(torch.ones((q_len, kv_len), device=query.device), diagonal=1).bool()
