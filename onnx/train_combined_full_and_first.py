#!/usr/bin/env python3
"""
Train encoder-decoder model on BOTH:
1. Full segments -> full transcriptions
2. First 1 second -> first word
Combined training for 100 epochs
"""
import json
import torch
...
def train_combined(model, segment_files, transcriptions, vocab, num_epochs=100, initial_lr=1e-3, min_lr=1e-6):
    ...
    for epoch in range(num_epochs):
        ...
        for i in indices:
            ...
            # Train TWICE per segment: once on full, once on first second

            # 1. Train on FULL SEGMENT -> full transcription
            audio_features_full, _ = extract_mel_features(seg_file)
            text_tokens_full = tokenize_text(text, vocab)
            audio_batch_full = audio_features_full.transpose(0, 1).unsqueeze(0).to(device)

            full_sequence = [1] + text_tokens_full + [2]  # <s> + tokens + </s>
            ...
            logits = model(mel_features=audio_batch_full, text_ids=input_ids)
            ...
            # 2. Train on FIRST 1 SECOND -> first word
            audio_features_1s, _ = extract_mel_features(seg_file, max_duration_seconds=1.0)
            first_word = text.split()[0] if text.split() else ""
            text_tokens_1s = tokenize_text(first_word, vocab)
            audio_batch_1s = audio_features_1s.transpose(0, 1).unsqueeze(0).to(device)

            full_sequence = [1] + text_tokens_1s + [2]  # <s> + first_word + </s>
            input_ids = torch.tensor([full_sequence[:-1]], dtype=torch.long, device=device)
            labels = torch.tensor([full_sequence[1:]], dtype=torch.long, device=device)

            logits = model(mel_features=audio_batch_1s, text_ids=input_ids)
            loss = criterion(logits.view(-1, logits.size(-1)), labels.view(-1))
        ...
        if (epoch + 1) % 50 == 0:
            ...
            with torch.no_grad():
                ...
                print(f"  🔹 Full sample: {' '.join(generated_words)}")
                print(f"  🔸 Expected: {transcriptions[0]}")

            # Test first second
            test_audio_1s, _ = extract_mel_features(segment_files[0], max_duration_seconds=1.0)
            test_audio_1s = test_audio_1s.transpose(0, 1).unsqueeze(0).to(device)
            with torch.no_grad():
                generated = model.generate(test_audio_1s, max_new_tokens=5, audio_duration_seconds=1.0)
                generated_ids = generated[0].tolist()
                if generated_ids and generated_ids[0] == 1:
                    generated_ids = generated_ids[1:]
                if 2 in generated_ids:
                ...
                print(f"  🔹 First-second sample: {' '.join(generated_words)}")
...
def main():
    ...
    baqara_03_text_path = os.path.join(segments_dir, "002-03.txt")
    with open(baqara_03_text_path, "r", encoding="utf-8") as f:
        baqara_03_transcriptions = [line.strip() for line in f if line.strip()]
    baqara_03_segments = sorted(glob.glob(os.path.join(segments_dir, "002-03-*.wav")))
    print(f"Loaded {len(baqara_03_transcriptions)} Al-Baqara part 3 transcriptions, {len(baqara_03_segments)} segments")

    # Combine all datasets
    all_transcriptions = fatiha_transcriptions + baqara_01_transcriptions + baqara_02_transcriptions + baqara_03_transcriptions
    all_segment_files = fatiha_segments + baqara_01_segments + baqara_02_segments + baqara_03_segments
    print(f"\n✓ Total combined: {len(all_transcriptions)} transcriptions, {len(all_segment_files)} segments")
    print("Training on BOTH: full segments + first 1 second per segment (50/50 random mix)")
    ...
    if os.path.exists(model_path):
        ...
        print("✓ Model loaded successfully! Training on combined full+first-second approach.")
    else:
        print("No existing model found. Starting with fresh weights for combined training.")

    # Train
    print(f"\nStarting training for 100 epochs on {len(all_segment_files)} segments (full + first-second mix)...\n")
