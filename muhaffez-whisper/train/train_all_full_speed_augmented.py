#!/usr/bin/env python3
"""
Train on ALL segments with speed perturbation augmentation
Usage: python3 train_all_full_speed_augmented.py

This script trains on the entire Quran-A dataset with speed perturbation:
- Speed perturbation: 0.9x, 1.0x, or 1.1x (varying speaking rate)

This makes the model robust to different speaking rates without changing pitch.
"""
import sys
import warnings
...
# ==============================================================
def apply_speed_perturbation(waveform, speed_factor):
    """
    Apply speed perturbation without changing pitch significantly
    ...
    # Faster speed = fewer samples, slower speed = more samples
    original_length = waveform.shape[1]

    # Use torchaudio's speed effect (changes duration without much pitch change)
    effects = [
        ["speed", str(speed_factor)],
        ["rate", "8000"]  # Keep sample rate constant at 8000 Hz
    ]

    # Apply effects
    perturbed, _ = torchaudio.sox_effects.apply_effects_tensor(
        waveform, 8000, effects
    )

    # Ensure same length as original (trim or pad)
    if perturbed.shape[1] > original_length:
        perturbed = perturbed[:, :original_length]
    elif perturbed.shape[1] < original_length:
        padding = original_length - perturbed.shape[1]
        perturbed = torch.nn.functional.pad(perturbed, (0, padding))
...
def load_and_augment_audio(audio_path, augment=True):
    """
    Load audio from mic folder and optionally apply speed perturbation

    Args:
        audio_path: path to .pt mel file (we derive .wav path from it)
        augment: whether to apply random speed perturbation
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"TRAINING WITH SPEED PERTURBATION")
    print(f"- Speed: 0.9x, 1.0x, 1.1x (varying speaking rate)")
    ...
    print(f"✓ Augmentation: Speed perturbation (0.9x-1.1x)")
