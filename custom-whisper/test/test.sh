...
echo "============================================================" | tee "$LOG_FILE"
echo "RUNNING BOTH TEST SUITES" | tee -a "$LOG_FILE"
echo "Dataset: $DATASET" | tee -a "$LOG_FILE"
echo "Surah parts to test: ${SURAH_PARTS[@]}" | tee -a "$LOG_FILE"
...
run_test_suite() {
    ...
    if python3 "$script" "$DATASET" "$surah_part" 2>&1 | tee -a "$LOG_FILE"; then
        ...
        echo "" | tee -a "$LOG_FILE"
        echo "ERROR: Test suite failed. Check $LOG_FILE for details." | tee -a "$LOG_FILE"
...
for SURAH_PART in "${SURAH_PARTS[@]}"; do
    echo "" | tee -a "$LOG_FILE"
    echo "════════════════════════════════════════════════════════════" | tee -a "$LOG_FILE"
    echo "TESTING SURAH PART: $SURAH_PART" | tee -a "$LOG_FILE"
    echo "════════════════════════════════════════════════════════════" | tee -a "$LOG_FILE"
    echo "" | tee -a "$LOG_FILE"

    run_test_suite "test_curriculum.py" "Curriculum Testing" "$SURAH_PART" || exit 1
    run_test_suite "test_full.py" "Full Segments Testing" "$SURAH_PART" || exit 1
done
...
echo "Total runs:    $TOTAL test suites" | tee -a "$LOG_FILE"
...
if [ $FAILED -gt 0 ]; then
    ...
    echo "" | tee -a "$LOG_FILE"
    echo "✓ All test suites completed successfully!" | tee -a "$LOG_FILE"
    exit 0
fi
